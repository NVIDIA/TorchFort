...
void ActorCriticMLPModel::setup(const ParamMap& params) {
  ...
  std::set<std::string> supported_params{"dropout", "encoder_layer_sizes", "actor_layer_sizes", "value_layer_sizes", "state_dependent_sigma", "log_sigma_init"};
  ...
  if (value_layer_sizes[value_layer_sizes.size() - 1] != 1) {
    THROW_INVALID_USAGE("ActorCriticMLPModel::setup: error, the value of the last element of value_layer_sizes has to be equal to one.");
  }
  
  // Construct and register submodules.
  for (int i = 0; i < encoder_layer_sizes.size() - 1; ++i) {
      encoder_layers.push_back(register_module("encoder_fc_" + std::to_string(i), torch::nn::Linear(encoder_layer_sizes[i], encoder_layer_sizes[i + 1])));
      encoder_biases.push_back(register_parameter("encoder_b_" + std::to_string(i), torch::zeros(encoder_layer_sizes[i + 1])));
  }
  int encoder_last_layer_size = encoder_layer_sizes[encoder_layer_sizes.size() - 1];

  // actor
  actor_layers.push_back(register_module("actor_fc_entry", torch::nn::Linear(encoder_last_layer_size, actor_layer_sizes[0])));
  ...
  for (int i = 0; i < actor_layer_sizes.size() - 2; ++i) {
    actor_layers.push_back(register_module("actor_fc_" + std::to_string(i), torch::nn::Linear(actor_layer_sizes[i], actor_layer_sizes[i + 1])));
  ...
  // mu layer
  actor_layers.push_back(register_module("actor_fc_mu", torch::nn::Linear(actor_last_layer_size, actor_layer_sizes[actor_layer_sizes.size() - 1])));
  actor_biases.push_back(register_parameter("actor_b_mu", torch::zeros(actor_layer_sizes[actor_layer_sizes.size() - 1])));
  // sigma layer
  if (state_dependent_sigma) {
    actor_layers.push_back(register_module("actor_fc_log_sigma", torch::nn::Linear(actor_last_layer_size, actor_layer_sizes[actor_layer_sizes.size() - 1])));
  }
  actor_biases.push_back(register_parameter("actor_b_log_sigma", torch::zeros(actor_layer_sizes[actor_layer_sizes.size() - 1])));

  // value
  value_layers.push_back(register_module("value_fc_entry", torch::nn::Linear(encoder_last_layer_size, value_layer_sizes[0])));
  ...
  for (int i = 0; i < value_layer_sizes.size() - 1; ++i) {
    value_layers.push_back(register_module("value_fc_" + std::to_string(i), torch::nn::Linear(value_layer_sizes[i], value_layer_sizes[i + 1])));
