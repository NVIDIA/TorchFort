...
void TD3System::loadModel(const std::string& checkpoint_dir) {
  using namespace torchfort;
  std::filesystem::path root_dir(checkpoint_dir);
...
void TD3System::trainStep(float& p_loss_val, float& q_loss_val) {
  ...
  train_td3(p_model_, p_model_target_, q_models_, q_models_target_, s, sp, a, ap, r, d, is_weights,
            static_cast<float>(std::pow(gamma_, nstep_)), rho_, td_errors, p_loss_val, q_loss_val, update_policy);
