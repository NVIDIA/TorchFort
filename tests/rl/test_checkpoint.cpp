...
void checkpoint_roundtrip(const std::string& system) {
  ...
  const std::string src = system + "_src";
  const std::string full = system + "_restore_full";
  const std::string weights = system + "_restore_weights";
  const std::string ckpt_dir = "/tmp/torchfort_rl_ckpt_" + system;
  ...
  CHECK_TORCHFORT(torchfort_rl_off_policy_create_system(weights.c_str(), config.c_str(), TORCHFORT_DEVICE_CPU,
...
void checkpoint_roundtrip_on_policy(const std::string& system) {
  // silence TorchFort's verbose/logging output for the duration of the test
  std::stringstream cout_buffer;
  CoutRedirect cout_redirect(cout_buffer.rdbuf());
  ...
  // ---------------- weights-only restore (load_model) ----------------
  CHECK_TORCHFORT(torchfort_rl_on_policy_create_system(weights.c_str(), config.c_str(), TORCHFORT_DEVICE_CPU,
                                                       TORCHFORT_DEVICE_CPU));
  // prediction before loading stems from a fresh (independent) initialization and differs from the reference
