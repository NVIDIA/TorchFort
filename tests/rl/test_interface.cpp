...
std::string createModifiedTD3Config(int state_size, int action_size) {
  ...
  if (config["policy_model"] && config["policy_model"]["parameters"] &&
      config["policy_model"]["parameters"]["layer_sizes"]) {
  ...
  if (config["critic_model"] && config["critic_model"]["parameters"] &&
      config["critic_model"]["parameters"]["layer_sizes"]) {
...
TEST_P(TorchFortInterface, DDPGTD3Predict) {
  ...
  // try a prediction
  torchfort_rl_off_policy_predict("test", state, state_shape.size(), state_shape.data(), action, action_shape.size(),
                                  action_shape.data(), TORCHFORT_FLOAT, 0);
...
TEST_P(TorchFortInterface, DDPGTD3PredictExplore) {
  ...
  torchfort_rl_off_policy_predict_explore("test", state, state_shape.size(), state_shape.data(), action,
                                          action_shape.size(), action_shape.data(), TORCHFORT_FLOAT, 0);
...
TEST_P(TorchFortInterface, DDPGTD3Evaluate) {
  ...
  // try a prediction with noise (explore)
  torchfort_rl_off_policy_evaluate("test", state, state_shape.size(), state_shape.data(), action, action_shape.size(),
                                   action_shape.data(), reward, 1, reward_shape.data(), TORCHFORT_FLOAT, 0);
...
TEST_P(TorchFortInterface, PPOPredict) {
  ...
  // try a prediction
  torchfort_rl_on_policy_predict("test", state, state_shape.size(), state_shape.data(), action, action_shape.size(),
                                 action_shape.data(), TORCHFORT_FLOAT, 0);
...
TEST_P(TorchFortInterface, PPOPredictExplore) {
  ...
  // try a prediction
  torchfort_rl_on_policy_predict_explore("test", state, state_shape.size(), state_shape.data(), action,
                                         action_shape.size(), action_shape.data(), TORCHFORT_FLOAT, 0);
...
TEST_P(TorchFortInterface, PPOEvaluate) {
  ...
  // try a prediction
  torchfort_rl_on_policy_evaluate("test", state, state_shape.size(), state_shape.data(), action, action_shape.size(),
                                  action_shape.data(), reward, 1, reward_shape.data(), TORCHFORT_FLOAT, 0);
...
INSTANTIATE_TEST_SUITE_P(TestShapes,         // Your instance label
                         TorchFortInterface, // Test fixture class
                         ::testing::Values(std::vector<std::vector<int64_t>>{{1}, {1}},
